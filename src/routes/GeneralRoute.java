...
public class GeneralRoute extends Route {	
		...
		LinkedList<String> params = new LinkedList<String>(Arrays.asList(path.split("/")));
			...
			if (op == null) {
				...
				if (op == null) {
					...
					int householdID;
					try {
						householdID = Integer.parseUnsignedInt(op);
					} catch (NumberFormatException e){
						error(xchg, ResponseCode.HOUSEHOLD_NOT_FOUND);
						return;
					}
	...
	private void handleLists(HttpExchange xchg, LinkedList<String> params) throws IOException {
		...
		if (op == null) {
			...
			int listID;
			try {
				listID = Integer.parseUnsignedInt(op);
			} catch (NumberFormatException e) {
				error(xchg, ResponseCode.LIST_NOT_FOUND);
				return;
			}
	...
	private void handleRecipes(HttpExchange xchg, LinkedList<String> params) throws IOException {
		...
		if (op == null) {
			...
			int recipeID;
			try {
				recipeID = Integer.parseUnsignedInt(op);
			} catch (NumberFormatException e) {
				error(xchg, ResponseCode.RECIPE_NOT_FOUND);
				return;
			}
